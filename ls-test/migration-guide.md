...
#### 5. SSL/TLS 基础配置
- 后端 TLS 配置（proxy-ssl-secret、proxy-ssl-name、proxy-ssl-server-name、proxy-ssl-verify）
...
| **TLS 配置** | ssl-ciphers、proxy-ssl-ciphers、proxy-ssl-protocols | `higress.io/ssl-*` 或 `proxy-ssl-*` 系列注解                         |
| **外部认证** | auth-url、auth-response-headers、auth-proxy-set-headers、auth-method | ext-auth WASM 插件（`forward_auth` 模式）                    |
...
#### 3. 外部认证相关（10 个）
- **不兼容原因**：Higress ext-auth WASM 插件功能覆盖有限,不支持认证缓存、keepalive 连接池配置、登录页重定向及部分高级配置项
- **影响注解**：
  - auth-signin（认证失败后重定向的登录页面 URL,ext-auth 插件不支持登录页重定向功能）
...
#### 5. 可观测性（3 个）
- **不兼容原因**：Envoy 的 tracing/日志机制在 HCM 级别配置,无法按路由粒度开关;没有独立的 rewrite 调试日志机制
- **影响注解**：
  - enable-opentelemetry（per-Ingress 粒度的 tracing 开关,Envoy tracing provider 配置在 HCM 级别,无法按路由粒度开关）
  - opentelemetry-trust-incoming-span（per-Ingress 粒度控制是否信任上游 trace context,Envoy 默认信任且不可按路由调整）
  - enable-rewrite-log（Envoy 无法按 Ingress 粒度输出 rewrite 匹配过程日志）

#### 6. Session Cookie 高级属性（4 个）
...
#### 11. SSL/TLS 透传（1 个）
- **不兼容原因**：Higress 源码中 `pkg/ingress/kube/annotations/` 未实现此注解的解析,Ingress 注解层面不支持 TLS 透传功能
- **影响注解**：
  - ssl-passthrough（SSL/TLS 透传,不在网关层解密,直接传递到后端）
...
#### 14. 限流突发倍数（1 个，新增）
- **不兼容原因**：Higress 限流插件不支持配置突发倍数,C++ 版使用令牌桶算法（天然允许瞬时突发到桶容量但无独立 burst 参数）,Go 版使用 Redis 固定窗口计数器（完全没有突发概念）
- **影响注解**：
  - limit-burst-multiplier（限流突发倍数因子,v0.24.0+ 引入,迁移时直接忽略）
...
| ssl-prefer-server-ciphers | Envoy 底层使用 BoringSSL，默认行为即为服务端优先选择加密套件顺序，无需额外配置                   |
| proxy-body-size         | Envoy 流式转发架构不会因大请求体导致内存爆炸，单连接内存上界仅为 `per_connection_buffer_limit_bytes`（默认 32KB），从"保护网关"角度该限制不再必要。如需在网关层拦截超大请求，可通过 WasmPlugin 在 `OnHttpRequestHeaders` 阶段检查 `Content-Length` 实现 |
...
| proxy-request-buffering | Envoy 天然流式处理，默认行为已满足大文件上传等场景需求                                            |
...
| session-cookie-change-on-failure | Envoy 的 outlier detection 和健康检查机制天然覆盖此注解的核心诉求（后端故障时自动切换到健康节点），最终效果等效 |
...
7. **部分兼容注解适配**：评估 `load-balance`(ewma)、`upstream-hash-by`(多变量组合)、`backend-protocol`(AJP/FCGI)等部分兼容注解的实际使用情况,按需调整;`ssl-passthrough` 已标记为不兼容,如需 TLS 透传功能需改用 Gateway API TLSRoute + Passthrough 模式
...
**A**：Envoy 有自己的缓冲机制，默认配置已能处理大多数场景。`proxy-body-size`（请求体大小限制）已归类为🔴无需迁移——Envoy 采用流式转发架构，单连接内存上界仅为 `per_connection_buffer_limit_bytes`（默认 32KB），不管请求体多大都不会导致内存爆炸，从"保护网关"角度该限制不再必要。如需在网关层拦截超大请求（不想让流量到达后端），可通过 WasmPlugin 在 `OnHttpRequestHeaders` 阶段检查 `Content-Length` 或在 `OnHttpRequestBody` 阶段累计 body 大小，超限返回 413 实现路由粒度的请求体限制。
...
- [Higress 注解列表](https://higress.io/docs/latest/user/annotation/)
- [Higress WASM 插件开发](https://higress.io/docs/latest/user/plugins/wasm-dev/wasm14/)
