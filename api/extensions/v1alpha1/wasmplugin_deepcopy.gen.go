...
func (in *WasmPlugin) DeepCopyInto(out *WasmPlugin) {
	...
	*out = *p
	if in.ResourceRefs != nil {
		out.ResourceRefs = make([]string, len(in.ResourceRefs))
		copy(out.ResourceRefs, in.ResourceRefs)
	}
	if in.ResourceTemplateSchema != nil {
		out.ResourceTemplateSchema = make([]*ResourceTemplateSchemaEntry, len(in.ResourceTemplateSchema))
		for i, entry := range in.ResourceTemplateSchema {
			if entry != nil {
				out.ResourceTemplateSchema[i] = entry.DeepCopy()
			}
		}
	}
}
...
// DeepCopyInto supports using ResourceTemplateSchemaEntry within kubernetes types.
func (in *ResourceTemplateSchemaEntry) DeepCopyInto(out *ResourceTemplateSchemaEntry) {
	*out = *in
}

// DeepCopy creates a new ResourceTemplateSchemaEntry.
